...
$sql="select article.*,contentclass.class_name from article,contentclass where article.class_id=contentclass.id order by subtime desc limit {$offset},{$length}";
$smt=$pdo->prepare($sql);
$smt->execute();
$rows=$smt->fetchAll();
 ?>
    ...
    <div class="container">
        ...
        <table class="table table-striped table-hover table-bordered">
            ...
            foreach ($rows as $row) {
                ...
                echo "<td><a target='_blank' href='content.php?p1={$row['id']}'>点击看内容</a></td>";
                echo "<td><img width='50' src='../../public/uploads/img/{$row['img']}'></td>";
                echo "<td>{$row['class_name']}</td>";
                echo "<td>{$row['subtime']}</td>";
                echo "<td><a href='edit.php?id={$row['id']}&img={$row['img']}' class='btn btn-success'>修改</a></td>";
                echo "<td><a href='delete.php?id={$row['id']}&img={$row['img']}' class='btn btn-danger delete'>删除</a></td>";
                echo "</tr>";
            }
            ?>
        </table>
